...
- Runs inside a throwaway container of the `ldapium:e2e` image (`docker run --rm`).
- Starts **no slapd daemon** and writes **no database files**.
...
- Executes OpenLDAP `slapadd -u -c` (dry-run, continue-on-error) against the
  provided LDIF to catch schema syntax and structural constraint errors.
- Generates a deterministic JSON report analyzing:
  ...
  - `duplicate_collisions`: Violations of the `unique` overlay constraint (`uid`, `mail`).
  ...
  - `errors`: Per-entry error messages from `slapadd` or schema evaluation.
...
For existing instances or bulk loads without restarting Kubernetes pods:
```bash
./scripts/restore.sh \
  ...
  --confirm-offline
```
Uses offline `slapadd` directly against storage volumes with no network overhead.
...
This architecture is continuously live-verified in
...
1. **Write Quiescence**: Set the source directory to read-only or restrict
   modifications to freeze state.
2. **Final Delta Sync**: Export the final LDIF diff from the source directory,
   validate via `scripts/migration-dryrun.sh`, and apply to ldapium.
...
4. **Downtime & Availability Evidence**:
   - Write availability during maintenance is measured using the write probe
     methodology from `.github/workflows/upgrade-e2e.yml`
     (`upgrade-write-probe`).
   - Read authentication continues via Keycloak session tokens and replica
     caching. Write downtime is strictly bounded by the final LDIF import duration.
