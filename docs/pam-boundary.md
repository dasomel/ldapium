...
- **Evidence integrity limits**: The audit export generated by `scripts/export-audit-log.sh`
  ...
  **These records are append-only files and database rows; they are NOT cryptographically
  signed, tamper-evident, or immutable.**
- If an attacker gains root access to the container or direct write access to the
  underlying persistent volume, audit files can be modified or truncated, and
  `cn=accesslog` entries can be deleted.
- **Operator requirement**: For regulatory compliance and non-repudiation, operators must
  stream container stdout immediately to an external, write-once/immutable SIEM or log
  aggregator.
